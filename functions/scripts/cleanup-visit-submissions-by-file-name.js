...
const CONFIRMATION = 'Bylaws_RIY-26-27';
const REMOVAL_REASON = 'Cleanup of accidental bulk upload: Bylaws_RIY 26-27.pdf';

const TARGET = Object.freeze({
  originalFileName: 'Bylaws_RIY 26-27.pdf',
...
function printUsage() {
  console.log([
    'Usage:',
    '  Dry run:',
    '    node functions/scripts/cleanup-visit-submissions-by-file-name.js',
    '',
    '  Execute:',
    '    node functions/scripts/cleanup-visit-submissions-by-file-name.js --execute --confirm=Bylaws_RIY-26-27',
    '',
    'Optional:',
...
function isTargetActiveSubmission(data) {
  return Boolean(
    data
    && data.originalFileName === TARGET.originalFileName
...
async function findMatchingSubmissions(db) {
  const snapshot = await db.collection('visitSubmissions')
    .where('originalFileName', '==', TARGET.originalFileName)
    .where('visitType', '==', TARGET.visitType)
    .where('uploadedByUid', '==', TARGET.uploadedByUid)
    .where('status', '==', TARGET.status)
    .get();

  const rawRecords = snapshot.docs
    .map((doc) => {
      const data = doc.data() || {};
      return {
        id: doc.id,
        submissionId: normalizeText(data.submissionId || doc.id, 160),
        data,
      };
    })
    .sort(sortRecords);
...
async function main() {
  ...
  console.log(`Target originalFileName: ${TARGET.originalFileName}`);
