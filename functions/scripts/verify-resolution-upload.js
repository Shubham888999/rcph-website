...
(async () => {
  ...
  const driveSource = fs.readFileSync(path.join(__dirname, '..', 'lib', 'resolution-drive.js'), 'utf8');
  const indexSource = fs.readFileSync(path.join(__dirname, '..', 'index.js'), 'utf8');
  for (const marker of ['resolutionPdfUploadSessions', "status: 'pending'", "status: 'uploading'", "status: 'uploaded'", "status: 'finalized'", "status: 'expired'", "status: 'cancelled'", 'proofHash', 'driveFileId', 'driveFolderKey']) assert.match(uploadSource, new RegExp(marker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  assert.match(uploadSource, /'failed'/);
  assert.match(uploadSource, /'conflict'/);
  assert.match(driveSource, /RCPH-resolution-source-\$\{safeNamePart\(resolutionId\)\}-\$\{safeNamePart\(uploadId\)\}\.pdf/);
  ...
  assert.doesNotMatch(uploadSource, /makePublic/);
