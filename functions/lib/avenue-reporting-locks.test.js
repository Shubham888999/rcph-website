...
test('submitBodEvent and updateBodEvent call avenue lock enforcement after payload normalization', () => {
  ...
  assert.match(indexSource, /exports\.updateBodEvent[\s\S]*const recoveredData = recoverReportingWindowIdForBodEventUpdate\(request\.data \|\| \{\}, bodEventData\);[\s\S]*const reportingWindow = await loadReportingWindowForBodEventUpdate\(eventId, recoveredData\);[\s\S]*const payloadData = withRecoveredReportingWindowId\(recoveredData, reportingWindow\);[\s\S]*const payload = normalizeBodEventPayload\(payloadData, \{[\s\S]*allowedMissingAvenues: allowedMissingAvenuesForReportingWindow\(reportingWindow\),[\s\S]*\}\);\s*await assertBodEventAvenuesUnlocked\(payload\.avenues\);/);
});
