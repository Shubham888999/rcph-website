...
function ResolutionForm({ value, onChange, meetings, roster, busy, submitLabel, onSubmit, onPreview, onNotice, onPersisted }) {
  ...
  return <form className="admin-form resolution-form" onSubmit={onSubmit}>
    ...
    <ResolutionPdfBuilder value={value} onChange={onChange} disabled={busy} onPreview={onPreview} onNotice={onNotice} onPersisted={onPersisted} />
...
function ResolutionCard({ item, busy, onEdit, onEditLayout, onOpen, onClose, onCancel, onView }) {
  ...
  return <article className={`resolution-admin-card is-${item.status}`}>
    ...
    <div className="resolution-admin-card__actions">
      {item.status === "draft" ? <><button disabled={busy} onClick={() => onEdit(item)}>Edit</button><button disabled={busy || (item.documentSourceMode === "uploadedPdf" && item.uploadedSource?.status !== "ready")} onClick={() => onOpen(item)}>Open voting</button></> : null}
...
function ResolutionDetails({ details, busy, onRefresh, onDownload, onRetry }) {
...
export default function ResolutionsModule({ uid, onNotice }) {
  ...
  return <>
    ...
    <section className="admin-panel"><h3>Create draft resolution</h3><ResolutionForm value={draft} onChange={setDraft} meetings={state.meetings} roster={state.roster} busy={busy || !state.meetings.length || !state.roster.length} submitLabel="Save draft" onSubmit={create} onPreview={() => downloadPreview(draft)} onNotice={onNotice} /></section>
