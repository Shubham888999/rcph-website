...
test("Member inspector shows protected profile details and labeled missing fields", () => {
  ...
  assert.match(
  source,
  /member\.normalizedProfileRid \|\| "Not recorded"/,
);
...
assert.match(source, /<dt>Dues paid<\/dt>/);
assert.match(
  source,
  /linkedProfile\.duesPaid === true \? "Paid" : "Not paid"/,
);
assert.doesNotMatch(source, /<dt>Roster RID<\/dt>/);
});

test("Members workspace uses one canonical RID display", () => {
  assert.match(
    source,
    /member\.normalizedProfileRid \? `RID \$\{member\.normalizedProfileRid\}` : "RID not recorded"/,
  );
...
test("Admin profile editor receives the dues-paid state", () => {
  assert.match(
    source,
