...
export function AnnouncementsModule({ uid, onNotice }) {
  ...
  function publish(event) {
    ...
    run("publish", () => adminCalls.publishAnnouncement(payload), "Announcement published through the server delivery system.").then((result) => { if (result) { setDraft(empty); load(true); } });
  }
  ...
  return <>
    <AdminModuleHeader title="Announcements" description="Publish targeted dashboard announcements with optional server-side email delivery. Production supports publish and history only." />
