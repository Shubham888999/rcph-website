...
export function normalizeMember(id, raw) { if (!id || !raw || typeof raw !== "object") return null; return { id, name: stripRotaractorPrefix(text(raw.name, 160)) || "Unnamed member", email: text(raw.email, 320).toLowerCase(), rid: text(raw.rid, 40), role: text(raw.role, 30), position: text(raw.position || raw.clubPosition, 180), active: raw.active !== false }; }
