...
export default function AdminDialog({ title, children, onClose, busy = false, className = "" }) {
  ...
  useEffect(() => {
    ...
    function keydown(event) { if (event.key === "Escape" && !busy) closeRef.current(); if (event.key !== "Tab") return; const items = [...dialog.querySelectorAll(selector)]; if (!items.length) return; const first = items[0], last = items.at(-1); if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } }
