...
export default function useAdminMutation({ uid, module, onNotice }) {
  ...
  const run = useCallback(async (operation, request, success) => {
    ...
    catch (error) { if (import.meta.env.DEV) console.error("Admin mutation failed.", adminDiagnostic(error, operation, module, uid)); onNotice?.({ type: "error", message: safeAdminError(error) }); return null; }
