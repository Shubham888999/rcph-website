...
export default function CouncilMemberCard({ member, active, onToggle, reduceMotion, buttonRef, index }) {
  ...
  return (
    ...
    >
      {expandable ? (
        <button ref={buttonRef} className="bod-member-card__trigger council-member-card__trigger" type="button" aria-expanded={active} aria-controls={detailId} aria-label={`${active ? "Close" : "Open"} profile for ${member.name}, ${member.role}`} onClick={() => onToggle(member)}>
          {cardContents}
        </button>
