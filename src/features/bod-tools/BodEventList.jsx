...
export default function BodEventList({ events, access, lockState, onDetails, onEdit, onArchive, onSync, onReset }) {
  ...
  return <ul className="bod-event-grid">{events.map((event) => <BodEventCard key={event.id} event={event} permissions={getBodEventPermissions(event, access, lockState)} onDetails={onDetails} onEdit={onEdit} onArchive={onArchive} onSync={onSync} />)}</ul>;
}
